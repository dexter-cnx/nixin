...
fn run_image<F>(f: F) -> *mut ImageBuffer
where
    F: FnOnce() -> Result<api::DevelopedImage, String>,
{
...
#[no_mangle]
pub extern "C" fn get_v7_version() -> *mut c_char {
    string_to_c("Nixin Studio raw-engine 8.0.0".into())
}
...
#[no_mangle]
pub extern "C" fn image_buffer_get_width(buf: *const ImageBuffer) -> u32 {
    if buf.is_null() {
        0
    } else {
        unsafe { (*buf).width }
...
#[no_mangle]
pub extern "C" fn image_buffer_get_height(buf: *const ImageBuffer) -> u32 {
    if buf.is_null() {
        0
    } else {
        unsafe { (*buf).height }
...
#[no_mangle]
pub extern "C" fn image_buffer_get_len(buf: *const ImageBuffer) -> usize {
    if buf.is_null() {
        0
    } else {
        unsafe { (*buf).len }
...
#[no_mangle]
pub extern "C" fn image_buffer_get_data(buf: *const ImageBuffer) -> *const u8 {
    if buf.is_null() {
        ptr::null()
    } else {
...
pub extern "C" fn get_last_error() -> *mut c_char {
    let s = LAST_ERROR
        .lock()
        .map(|e| e.clone())
        .unwrap_or_else(|_| "LAST_ERROR poisoned".into());
    string_to_c(s)
}
...
#[no_mangle]
pub extern "C" fn check_engine() -> c_int {
    if LAST_ERROR.lock().is_ok() {
        1
    } else {
        0
    }
...
pub extern "C" fn develop_raw_with_settings(
    ...
    run_image(|| {
        api::apply_develop_settings(path, develop_settings(exposure, temperature, contrast))
    })
}

#[no_mangle]
pub extern "C" fn detect_subject_mask(
    path: *const c_char,
    x: i32,
    y: i32,
    has_click: c_int,
...
#[no_mangle]
pub extern "C" fn apply_lut_file(
    path: *const c_char,
    lut_path: *const c_char,
    strength: f32,
) -> *mut ImageBuffer {
    let path = match cstr_to_string(path) {
        Ok(v) => v,
    ...
    let lut = match cstr_to_string(lut_path) {
        Ok(v) => v,
        Err(e) => {
            set_last_error(e);
            return ptr::null_mut();
...
#[no_mangle]
pub extern "C" fn export_jpeg_with_quality(
    path: *const c_char,
    dest: *const c_char,
    quality: u8,
) -> *mut c_char {
...
pub extern "C" fn export_jpeg_with_settings(
    ...
    let dest = match cstr_to_string(dest) {
        Ok(v) => v,
        Err(e) => {
            set_last_error(e);
            return ptr::null_mut();
