...
impl Default for DevelopSettings {
    fn default() -> Self {
        Self { exposure: 0.0, temperature: 0.0, contrast: 1.0 }
    }
}
...
impl Default for ExportOptions {
    fn default() -> Self {
        Self { quality: 90, rating: None, label: None }
    }
}
...
fn extract_best_embedded_jpeg(bytes: &[u8]) -> Result<DynamicImage, String> {
    ...
    for start in starts {
        ...
        while j + 1 < bytes.len() {
            if bytes[j] == 0xFF && bytes[j + 1] == 0xD9 {
                ...
                if let Ok(img) = image::load_from_memory_with_format(candidate, image::ImageFormat::Jpeg) {
    ...
    best.map(|(_, img)| img).ok_or_else(|| "Embedded JPEG markers found, but none decoded".into())
}

pub fn apply_develop_settings(path: String, settings: DevelopSettings) -> Result<DevelopedImage, String> {
...
fn apply_settings_to_rgba(img: &mut RgbaImage, settings: DevelopSettings) -> Result<(), String> {
    if !settings.exposure.is_finite() || !settings.temperature.is_finite() || !settings.contrast.is_finite() {
...
pub fn detect_subject_mask_internal(path: String, click: Option<(i32, i32)>) -> Result<MaskResult, String> {
    ...
    if let Some((x, y)) = click {
        ...
        let border_avg = if border_count == 0 { 0.0 } else { border_sum as f32 / border_count as f32 };
        for y in 0..h {
            for x in 0..w {
    ...
    Ok(MaskResult { overlay: mask_overlay(&rgba, &mask, [255, 70, 90]), mask })
}
...
pub fn detect_sky_mask_internal(path: String) -> Result<MaskResult, String> {
    ...
    for y in 0..h {
        let vertical_prior = if h <= 1 { 1.0 } else { 1.0 - 0.8 * (y as f32 / (h - 1) as f32) };
        for x in 0..w {
            let p = rgba.get_pixel(x, y).0;
            let r = p[0] as f32;
            let g = p[1] as f32;
...
pub fn apply_lut_internal(path: String, lut_path: String, strength: f32) -> Result<DevelopedImage, String> {
    if !strength.is_finite() { return Err("LUT strength must be finite".into()); }
    let strength = strength.clamp(0.0, 1.0);
    let lut = parse_cube(&fs::read_to_string(&lut_path).map_err(|e| format!("LUT read failed: {e}"))?)?;
    let mut rgba = load_embedded_preview(&path)?.to_rgba8();
    for p in rgba.pixels_mut() {
        let src = [p[0] as f32 / 255.0, p[1] as f32 / 255.0, p[2] as f32 / 255.0];
...
pub fn parse_cube(text: &str) -> Result<Lut3D, String> {
    ...
    for raw in text.lines() {
        ...
        if let Some(rest) = line.strip_prefix("LUT_3D_SIZE") {
            size = Some(rest.trim().parse::<usize>().map_err(|_| "Invalid LUT_3D_SIZE")?);
            continue;
        }
        ...
        if parts.len() == 3 {
            ...
            if vals.iter().any(|v| !v.is_finite()) { return Err("Non-finite LUT value".into()); }
            data.push(vals);
        }
    }
...
fn parse_triplet(rest: &str) -> Result<[f32; 3], String> {
    ...
    if v.len() != 3 { return Err("Expected 3 values".into()); }
    Ok([
...
pub fn sample_lut_trilinear(lut: &Lut3D, rgb: [f32; 3]) -> Result<[f32; 3], String> {
    if rgb.iter().any(|v| !v.is_finite()) { return Err("Non-finite LUT input".into()); }
    let n = lut.size as f32 - 1.0;
    let x = rgb[0].clamp(0.0, 1.0) * n;
    let y = rgb[1].clamp(0.0, 1.0) * n;
    let z = rgb[2].clamp(0.0, 1.0) * n;
    let r0 = x.floor() as usize; let r1 = (r0 + 1).min(lut.size - 1); let rd = x - r0 as f32;
    let g0 = y.floor() as usize; let g1 = (g0 + 1).min(lut.size - 1); let gd = y - g0 as f32;
    let b0 = z.floor() as usize; let b1 = (b0 + 1).min(lut.size - 1); let bd = z - b0 as f32;
    let at = |r: usize, g: usize, b: usize| lut.data[r + g * lut.size + b * lut.size * lut.size];
    let c000 = at(r0,g0,b0); let c100 = at(r1,g0,b0); let c010 = at(r0,g1,b0); let c110 = at(r1,g1,b0);
    let c001 = at(r0,g0,b1); let c101 = at(r1,g0,b1); let c011 = at(r0,g1,b1); let c111 = at(r1,g1,b1);
    let c00 = lerp3(c000,c100,rd); let c10 = lerp3(c010,c110,rd);
    let c01 = lerp3(c001,c101,rd); let c11 = lerp3(c011,c111,rd);
    let c0 = lerp3(c00,c10,gd); let c1 = lerp3(c01,c11,gd);
    Ok(lerp3(c0,c1,bd))
}

fn lerp3(a:[f32;3], b:[f32;3], t:f32)->[f32;3] { [a[0]+(b[0]-a[0])*t, a[1]+(b[1]-a[1])*t, a[2]+(b[2]-a[2])*t] }

pub fn export_developed_jpeg(path: String, dest: String, settings: DevelopSettings, options: ExportOptions) -> Result<String, String> {
    ...
    encoder.write_image(&rgb, developed.width, developed.height, ExtendedColorType::Rgb8)
    ...
    if options.rating.is_some() || options.label.is_some() {
        ...
        f.write_all(xmp.as_bytes()).map_err(|e| format!("Write XMP failed: {e}"))?;
    }
    Ok(dest)
}

pub fn build_xmp(rating: Option<i32>, label: Option<&str>) -> String {
    let rating_attr = rating.map(|v| format!(" xmp:Rating=\"{}\"", v)).unwrap_or_default();
    let label_attr = label.map(|v| format!(" xmp:Label=\"{}\"", xml_escape(v))).unwrap_or_default();
...
pub struct TetheredWatcher { pub watch_folder: PathBuf }
        ...
        if let Ok(read) = fs::read_dir(&self.watch_folder) {
            for e in read.flatten() {
                if let Some(s) = e.path().to_str() {
                    if !known.contains(s) { out.push(s.to_string()); }
                }
            }
        }
        out.sort();
...
fn from_rgba(img: RgbaImage) -> DevelopedImage { let (width,height)=img.dimensions(); DevelopedImage { width,height,data:img.into_raw() } }
fn brightness(p:[u8;4])->u8 { ((p[0] as u16 + p[1] as u16 + p[2] as u16)/3) as u8 }
fn mask_overlay(base:&RgbaImage, mask:&[u8], color:[u8;3])->DevelopedImage {
    let (w,h)=base.dimensions(); let mut out=base.clone();
    for (i,p) in out.pixels_mut().enumerate() { if mask.get(i).copied().unwrap_or(0)>0 { p[0]=((p[0] as u16+color[0] as u16)/2) as u8; p[1]=((p[1] as u16+color[1] as u16)/2) as u8; p[2]=((p[2] as u16+color[2] as u16)/2) as u8; p[3]=255; } }
    DevelopedImage { width:w,height:h,data:out.into_raw() }
}
fn rgba_to_rgb(rgba:&[u8])->Vec<u8> { rgba.chunks_exact(4).flat_map(|p| [p[0],p[1],p[2]]).collect() }
