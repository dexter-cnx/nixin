...
impl CatalogAsset {
    /// Mirrors the production catalog rule: a managed path takes precedence
    /// over the original source path without changing stable catalog identity.
...
/// Framework-neutral domain boundary for a native desktop catalog consumer.
///
/// GPUI must depend on this contract rather than Hive, Dart storage objects, or
/// any concrete database. A future production adapter can bridge this contract
/// to the authoritative Workplaces/catalog implementation chosen after the
/// architecture review.
...
/// S3 test adapter. It deliberately implements production-like domain
/// semantics without representing the future persistence implementation.
...
impl CatalogRepository for SyntheticCatalogRepository {
    ...
    fn set_active_workplace(
        ...
        if !self.workplaces.iter().any(|workplace| workplace.id == workplace_id) {
