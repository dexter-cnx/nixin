...
const MISSING_STRIDE: usize = 17;
const RECENT_IMPORT_COUNT: usize = 500;
...
impl CatalogView {
    fn label(self) -> &'static str {
        match self {
            Self::AllPhotos => "All photos",
            Self::Missing => "Missing",
            Self::RecentImports => "Recent imports",
        }
    }
}
...
impl DextryxSpike {
    ...
    fn catalog_button(
        ...
        let selected = self.catalog_view == view;
        div()
            .id(id)
            .px_3()
            .py_2()
            ...
            .hover(|this| this.bg(rgb(0x25282e)))
            .child(label)
            .on_click(cx.listener(move |this, _, _, cx| {
                this.set_catalog_view(view, cx);
            }))
    }
    ...
    fn catalog_asset_count(&self) -> usize {
        match self.catalog_view {
            CatalogView::AllPhotos => S2_ASSET_COUNT,
            CatalogView::Missing => S2_ASSET_COUNT.div_ceil(MISSING_STRIDE),
            CatalogView::RecentImports => RECENT_IMPORT_COUNT.min(S2_ASSET_COUNT),
        }
    }

    fn asset_for_catalog_position(&self, position: usize) -> Option<usize> {
        let asset_ix = match self.catalog_view {
            CatalogView::AllPhotos => position,
            CatalogView::Missing => position.checked_mul(MISSING_STRIDE)?,
            CatalogView::RecentImports => {
                let start = S2_ASSET_COUNT.saturating_sub(RECENT_IMPORT_COUNT);
                start.checked_add(position)?
            }
        };

        (asset_ix < S2_ASSET_COUNT).then_some(asset_ix)
    }

    fn first_asset_for_catalog(&self) -> usize {
        self.asset_for_catalog_position(0).unwrap_or(0)
    }

    fn set_catalog_view(&mut self, view: CatalogView, cx: &mut Context<Self>) {
        if self.catalog_view == view {
            return;
        }
        ...
        self.selected_asset = self.first_asset_for_catalog();
        self.status = format!(
            "S3 catalog: {} — {} assets — selected stable asset id {}",
            view.label(),
            self.catalog_asset_count(),
            self.selected_asset + 1
        );
        cx.notify();
    }

    fn max_filmstrip_scroll(&self) -> f32 {
        (self.catalog_asset_count() as f32 * FILMSTRIP_STRIDE - FILMSTRIP_VIEW_WIDTH).max(0.0)
    }

    fn filmstrip_visible_positions(&self) -> std::ops::Range<usize> {
        let count = self.catalog_asset_count();
    ...
    fn schedule_visible_thumbnails(
        &mut self,
        visible_assets: impl IntoIterator<Item = usize>,
        cx: &mut Context<Self>,
    ) {
        self.thumbnail_queue.clear();
    ...
    fn filmstrip_asset(
        ...
        div()
            ...
            .on_click(cx.listener(move |this, _, _, cx| {
                ...
                this.status = format!(
                    "{} selection: stable asset id {}",
                    this.catalog_view.label(),
                    asset_ix + 1
                );
                cx.notify();
            }))
    ...
    fn s2_horizontal_filmstrip(&mut self, cx: &mut Context<Self>) -> impl IntoElement {
        let positions = self.filmstrip_visible_positions();
        let visible_pairs = positions
            .filter_map(|position| {
                self.asset_for_catalog_position(position)
                    .map(|asset_ix| (position, asset_ix))
            })
            .collect::<Vec<_>>();
                ...
                div()
                    ...
                    .child(
                        div()
                            ...
                            .child(self.catalog_button("catalog-all", "All photos", CatalogView::AllPhotos, cx))
                            .child(self.catalog_button("catalog-missing", "Missing", CatalogView::Missing, cx))
                            .child(self.catalog_button("catalog-recent", "Recent imports", CatalogView::RecentImports, cx))
                            .child(Self::panel_label("S2 HARNESS"))
                            .child("5,000 stable assets")
                            .child("horizontal virtualization")
                            .child("4 thumbnail workers")
                            .child("128 thumbnail cache")
                            .child(Self::panel_label("S3 STATE"))
                            .child("Catalog filters are live")
                            .child("Persistence adapter: next"),
                    )
                    .child(
                        div()
                            .flex()
                                ...
                                div()
                                    ...
                                    .child(
                                        div()
                                            ...
                                            .child("S2/S3 virtualized Filmstrip — scroll / trackpad")
                        ...
                        div()
                            ...
                            .child(
                                div()
                                    .text_sm()
                                    .text_color(rgb(0x858a94))
                                    .child("Visible Filmstrip range + 3-item overscan only. Synthetic thumbnails run on GPUI background executor with max 4 in-flight jobs."),
                            )
                            .child(div().text_sm().text_color(rgb(0x858a94)).child(thumb_status))
                            .child(Self::panel_label("S3 — STATE SHELL"))
                            .child(
                                div()
                                    .text_sm()
                                    .text_color(rgb(0x858a94))
                                    .child("Catalog navigation now changes the Filmstrip dataset while preserving stable asset IDs. No persistence ownership has moved yet."),
                            ),
                    ),
            )
    }
        ...
        cx.open_window(
            ...
            |_window, cx| {
                cx.new(|_cx| DextryxSpike {
                    ...
                    catalog_view: CatalogView::AllPhotos,
                    selected_asset: 0,
                    ...
                    status: "S1 passed. S2 final validation + S3 catalog state shell ready."
                        .to_string(),
                })
            },
        )
